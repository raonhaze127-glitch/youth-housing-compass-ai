...
기본 `npm run dev`는 저장된 마지막 실공고 스냅샷을 사용하고, 스냅샷도 없을 때만 출품용 샘플 데이터로 동작합니다. `npm run dev:full`은 별도 FastAPI 서비스가 기관 원본을 직접 수집합니다. 구조화된 공고문 분석 결과는 추천 카드에 사용하지만, 원문 즉석 해석 버튼과 경쟁률·가점·변동 추적 UI는 신뢰도 검증 전까지 숨깁니다. 문서의 에이전트 구조는 역할을 구분한 설계이며 독립적으로 판단하는 다중 에이전트 시스템을 의미하지 않습니다.
