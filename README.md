...
기본 `npm run dev`는 공모전 샘플 데이터로 동작합니다. `npm run dev:full`은 별도 FastAPI 서비스가 기관 원본을 직접 수집합니다. 현재 대화와 공고 해석은 규칙 및 추출 섹션 기반이며 외부 LLM API를 호출하지 않습니다. 문서의 에이전트 구조는 역할을 구분한 설계이며 독립적으로 판단하는 다중 에이전트 시스템을 의미하지 않습니다.
...
`dev:full`은 직접 수집 FastAPI와 Next.js를 함께 연결합니다. `DATA_GO_KR_API_KEY`가 없으면 키가 필요 없는 SH·GH 채널만 수집하고 청약홈·LH는 건너뜁니다.
