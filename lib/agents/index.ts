...
const HOUSING_TERMS =
  /공공주택|공공임대|공공분양|통합공공임대|영구임대|국민임대|행복주택|장기전세|매입임대|전세임대|분양전환|주거지원사업/;
const HOUSING_TYPE_DEFINITION_QUESTION =
  /^\s*[“"'‘’]?(?:공공주택|공공임대|공공분양|통합공공임대|영구임대|국민임대|행복주택|장기전세|매입임대|전세임대|분양전환공공임대|분양전환|주거지원사업)(?:은|는|이란|란|이|가)?\s*(?:뭐야|무엇|뜻|의미|설명)?\s*[?？]?\s*[”"'‘’]?\s*$/;
...
function policyAnswer(message: string) {
  ...
  if (/주거지원사업|긴급주거|주거취약/.test(message)) {
    return "주거지원사업은 일반 청약형 주택 유형이라기보다 긴급한 주거위기나 쪽방·고시원·비닐하우스 등 취약한 주거환경에 놓인 사람을 공공임대나 전세임대와 연결하는 주거복지 사업입니다. 위기상황, 현재 거처, 지원대상 여부와 지자체·기관 연계 절차를 함께 확인해야 합니다.";
  }
  if (/분양전환공공임대|분양전환|분납\s*임대|50년\s*공공임대/.test(message)) {
    return "분양전환공공임대는 5년·6년·10년 등 일정 기간 임대로 거주한 뒤 분양전환을 검토할 수 있는 공공임대 유형입니다. 분납 임대는 임대기간 중 주택가격 일부를 단계적으로 납부하는 방식이고, 50년 공공임대는 분양전환되지 않는 장기 임대입니다. 공고별 분양전환 여부와 가격 기준을 반드시 확인해야 합니다.";
  }
  if (/장기전세/.test(message)) {
    return "장기전세는 공공기관이 공급하는 임대주택을 전세 계약 방식으로 장기간 거주할 수 있게 하는 유형입니다. LH 안내 기준으로 20년 범위에서 공급될 수 있고, 무주택세대구성원 여부와 소득·자산·자동차 기준을 확인합니다. 월 임대료보다 보증금 부담이 중요하므로 공고의 보증금 조건을 꼭 봐야 합니다.";
  }
  if (/매입임대/.test(message)) {
    return "매입임대는 LH 같은 공공기관이 기존 주택을 매입한 뒤 청년, 신혼부부, 저소득층, 고령자 등에게 시세보다 저렴하게 다시 임대하는 방식입니다. 청년 매입임대, 신혼·신생아 매입임대, 다자녀 매입임대처럼 세부 유형이 나뉘므로 먼저 대상 유형을 확인해야 합니다.";
  }
  if (/전세임대/.test(message)) {
    return "전세임대는 입주자가 살 집을 찾으면 LH 등 공공기관이 집주인과 전세계약을 맺고, 그 집을 입주자에게 저렴하게 다시 임대하는 방식입니다. 청년 전세임대, 신혼·신생아 전세임대, 기존주택 전세임대처럼 세부 유형이 나뉘므로 대상 유형과 지원한도를 먼저 확인해야 합니다.";
  }
  if (/영구임대/.test(message)) {
    return "영구임대는 기초생활수급자, 국가유공자, 한부모가족 등 주거지원 필요도가 높은 계층에게 낮은 임대료로 공급하는 사회복지적 성격의 공공임대주택입니다. LH 안내 기준으로 임대조건은 시세의 약 30% 수준으로 설명됩니다. 실제 신청 가능성은 복지 자격, 무주택 여부, 지역 기준과 모집공고를 함께 확인해야 합니다.";
  }
  if (/공공임대/.test(message)) {
    return "LH 청약플러스의 공공임대 안내는 주로 5년·6년·10년 임대 후 분양전환을 검토하는 공공임대, 분납 임대주택, 50년 공공임대를 포함합니다. 일부는 임대기간 종료 후 분양전환이 연결될 수 있지만, 50년 공공임대처럼 분양전환되지 않는 유형도 있으므로 공고 유형을 먼저 확인해야 합니다.";
  }
  if (/종류/.test(message) || /차이/.test(message)) {
    return "대표 공공임대 유형에는 통합공공임대, 영구임대, 국민임대, 행복주택, 장기전세, 매입임대, 전세임대, 분양전환공공임대가 있습니다. 주거지원사업은 주택 유형이라기보다 주거위기·주거취약 상황을 지원하는 사업 묶음입니다. 대상계층, 임대기간, 소득·자산 기준과 선정방식이 다르므로 관심 유형을 말씀해주시면 차이를 좁혀 설명할 수 있습니다.";
  }
  if (/행복주택/.test(message)) {
    return "행복주택은 대학생, 청년, 신혼부부, 한부모가족 등 생애 초기 계층과 고령자·주거급여수급자의 주거비 부담을 낮추기 위해 공급하는 공공임대주택입니다. 대중교통이 편리하거나 직장·학교와 가까운 곳에 공급되는 경우가 많고, 계층별로 소득·자산 기준과 최대 거주기간이 다릅니다. 최종 판단은 모집공고문 원문 기준으로 확인해야 합니다.";
  }
  if (/국민임대/.test(message)) {
    return "국민임대는 무주택 저소득 가구가 시세보다 낮은 임대료로 장기간 거주할 수 있도록 공급하는 공공임대주택입니다. 대표적으로 30년 장기 임대이고 분양전환은 되지 않습니다. 보통 무주택세대구성원 여부, 소득, 총자산, 자동차 기준을 함께 확인하며 최종 판단은 모집공고문 원문 기준으로 확인해야 합니다.";
  }
