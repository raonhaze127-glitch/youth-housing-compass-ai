...
function isPublicRecruitmentNotice(item: AnnouncementApiItem) {
  ...
  const publicApplyhome =
    item.organization === "청약홈" &&
    (item.metadata?.house_secd === "01" || item.metadata?.house_secd_name === "국민");
  return (
    (["LH", "SH", "GH"].includes(item.organization) || publicApplyhome) &&
    (included || publicApplyhome) &&
    !excluded &&
    !nonHousing
  );
}
