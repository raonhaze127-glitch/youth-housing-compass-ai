...
| 이름 | 기본값 | 설명 |
|---|---|---|
| `ANNOUNCEMENT_SOURCE` | `sample` | `sample` 또는 `k_apt_alert` |
...
실공고 API 주소 같은 로컬 설정은 Git에 포함되지 않는 `services/announcement-api/.env.local`에 저장합니다. `.env.example`을 복사하면 서비스 시작 시 자동으로 읽습니다.
...
확장 엔드포인트는 `K_APT_ALERT_API_BASE_URL`이 설정된 경우에만 활성화됩니다. 결과에 포함된 실제값과 통계 추정값, 일정 미확인 상태를 클라이언트에서 구분해야 합니다.
