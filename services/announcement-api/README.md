...
- `direct`: LH 공개 API, SH 게시판, GH 청약센터와 청약홈 공공 채널에서 공공주택을 직접 수집하는 모드
...
기본 서비스 범위는 LH·SH·GH 공공주택과 청약홈 APT 국민주택·공공지원민간임대입니다. `INCLUDE_PRIVATE_HOUSING=false`에서는 청약홈 공공 채널만 수집·노출하며, 일반 민영주택과 나머지 청약홈 채널은 SQLite에 과거 데이터가 남아 있어도 목록·변동·원문 조회에서 제외합니다.
